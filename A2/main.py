...
usedTriangles =[] # all triangles included
...
def buildTristrips( triangles ):
    ...
    # Start each strip through every triangle in order of adjacent triangles, starting with corners, then edges
    for triangle in triangles:
        if len(triangle.adjTris) == 1:
            if triangle.nextTri == None and triangle.prevTri == None:
                pushTri(triangle)
    
    for triangle in triangles:
        if len(triangle.adjTris) == 2:
            if triangle.nextTri == None and triangle.prevTri == None:
                pushTri(triangle)

    for triangle in triangles:
        if len(triangle.adjTris) == 3:
            if triangle.nextTri == None and triangle.prevTri == None:
                pushTri(triangle)

    # find number of "heads" in the triangle mesh, all heads have their nextTri == None
    for triangle in triangles:
        if triangle.nextTri == None:
            stripCount += 1
    print( 'Generated %d tristrips' % stripCount )

# FUNCTION: push Tri
# DESC: This recursive function builds a triangle strip using a greedy based algorithm
# PRE: list of Triangle class
# POST: Returns either 1(Strip Created) or 2(Created a strip that joins with a prexisting strip)

def pushTri(currTriangle):

    #print("Current Triangle: " + str(currTriangle) + "\nAdjacents: " + str(currTriangle.adjTris))

    adj = None
    adjList = validAdjacents(currTriangle)
    
    # Case 1: we reached the end of the line and we stop recursion calls
    if not adjList:
        usedTriangles.append(currTriangle)
        #display(True)
        return
    
    # Case 2: we have a possible adjacent we can add
    else:
        adj = findNextTri(adjList) # search for a possible adjacent
        linkTriangles(currTriangle,adj)
        usedTriangles.append(currTriangle)
        pushTri(adj)

# FUNCTION: validAdjacents
# DESC: This checks every triangle in a list to see if it is linkable (checks if its in our usedTriangles list)
# PRE: list of Triangle class (triangle.adjTris specifically)
# POST: returns a list of triangles not in adjTris
def validAdjacents(adjList):
    validAdjTris = []                   # to store the return value
    for adj in adjList.adjTris:
        if adj not in usedTriangles:
...
# DESC: This recursive function builds a triangle strip using a greedy based algorithm
# PRE: list of Triangle class (triangle.adjTris specifically)
# POST: triangle class, (candidate to link to our current triangle)
def findNextTri(adjList):
    nextTri = adjList[0]                    # since input is guaranteed to be NON-EMPTY, we can initialize the default shortest to the first element

    for potentialTriangle in adjList:
        currLen = len(validAdjacents(potentialTriangle))
        if currLen < len(validAdjacents(nextTri)):
...
# FUNCTION: findNextTri
# DESC: links two triangles together by updating their nextTri and prevTri
# PRE: triB is the new head of the strip, triA is the prev head of the strip
# POST: None
def linkTriangles(triA, triB):
    triA.nextTri = triB
    triB.prevTri = triA
    return
